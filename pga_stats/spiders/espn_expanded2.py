...
from pga_stats.items import expanded2
import time
...
class PGAStatsSpider(scrapy.Spider):
  ...
  custom_settings = {
          'ITEM_PIPELINES': {
              'pga_stats.pipelines.expanded2': 600
          }
    }
...
function main(splash, args)
  ...
  def start_requests(self):
    ...
    urls = ['http://www.espn.com/golf/statistics/_/year/{}/type/expanded2/count/{}'.format(i,x) for i in range(2015, 2020) for x in [1,41,81,121,161,201,241,281]]
    print('urls',urls)
  ...
  def parse(self, response):
    ...
    #print('year', year)
      for player in sel.xpath("//tr[contains(@class,'oddrow') or contains(@class,'evenrow')]"):
        loader = ItemLoader(item=expanded2(), selector=player, response=response)
        ...
        loader.add_xpath('eagles',".//child::td[4]/text()")
        loader.add_xpath('birdies',".//child::td[5]/text()")
        loader.add_xpath('pars',"..//child::td[6]/text()")
        loader.add_xpath('bogeys',".//child::td[7]/text()")
        loader.add_xpath('birdiesPerRound',".//child::td[8]/text()")
        loader.add_xpath('holesPerEagle',".//child::td[9]/text()")
